...
class Event(Message):
    ...
    def decode(self):
        ...
        for k,v in self.items():
            del self[k]
            self[k] = urllib.parse.unquote(v)    # update:  urllib.unquote() (to) urllib.parse.unquote()
    ...
    def as_string(self, unixfrom=False):
        ...
        Generator instance.
        """
        from email.generator import Generator    # update: email.Generator (to) email.generator
        from io import StringIO    # update from cStringIO (to) from io
        fp = StringIO()
...
class FSProtocol(basic.LineReceiver):
    ...
    #Call management
    def apiOriginate(self, url, application='', appargs='', extension='', dialplan='',
                     context='', cidname='', cidnum='', timeout='', channelvars={}, background=jobType):
    ...
    def playAndGetDigits(self, min, max, tries=3, timeout=4000,  terminators='#',
                         filename='', invalidfile='', varname='', regexp='\d', uuid='', lock=True):
    ...
    def _checkPlaybackResult(self, event, finalDF, varname):        
        if event['Application'] == "play_and_get_digits":
            self.deregisterEvent(finalDF.ecb)
            if ("variable_"+varname) in event:    # update: event.has_key("variable_"+varname) (to) in
    ...
    def record(self, path, time_limit_secs=' ', silence_thresh=' ',
               silence_hits=' ', terminators='', uuid='', lock=True):
